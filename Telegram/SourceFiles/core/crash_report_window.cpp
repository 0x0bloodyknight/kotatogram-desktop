...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? qsl("Prestongram") : title);
...
, _log(this)
, _close(this) {
	_label.setText(qsl("Could not start Prestongram Desktop!\nYou can see complete log below:"));
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(qsl("Last time Prestongram Desktop was not closed properly."));
	} else {
		_label.setText(qsl("Last time Prestongram Desktop crashed :("));
	}
...
void LastCrashedWindow::onCheckingFinished() {
	...
	if (result == "Old") {
		_pleaseSendReport.setText(qsl("This report is about some old version of Prestongram Desktop."));
		_sendingState = SendingTooOld;
		updateControls();
		return;
	} else if (result == "Unofficial") {
		_pleaseSendReport.setText(qsl("You use some custom version of Prestongram Desktop."));
...
void LastCrashedWindow::updateControls() {
	...
	QRect scr(QApplication::primaryScreen()->availableGeometry());
	QSize s(2 * padding + QFontMetrics(_label.font()).width(qsl("Last time Prestongram Desktop was not closed properly.")) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
